...
def get_confirm_trip_use_case(background_tasks: BackgroundTasks, db_session: AsyncSession = Depends(get_db)) -> ConfirmTripPort:
    smtp_email_sender = SMTPEmailSender()
    notification_sender = BackgroundTaskNotificationSender(
        background_tasks, smtp_email_sender)
    email_message = f'Para confirmar a sua participação, clique em: http://{settings.APP_HOST}:{settings.APP_PORT}/api/trips'
    trip_repo = SqlAlchemyTripRepository(db_session)
    uow = SqlAlchemyUnitOfWork(db_session)
